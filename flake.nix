{
  description = "A flake for building eepp projects";

  inputs = {
    nixpkgs.url = "github:NixOS/nixpkgs/nixpkgs-unstable";
    self.submodules = true;
  };

  outputs = { self, nixpkgs }:
    let
      systems = [
        "x86_64-linux"
        "aarch64-linux"
      ];

      # Local paths for submodules
      efswPath = builtins.path ./src/thirdparty/efsw;
      soil2Path = builtins.path ./src/thirdparty/soil2;
      premakeNinjaPath = builtins.path ./premake/premake-ninja;
      premakeCMakePath = builtins.path ./premake/premake-cmake;
    in
    {
      packages = builtins.listToAttrs (
        map (system: {
          name = system;
          value =
            let
              pkgs = import nixpkgs { inherit system; };
              eepp_pkgs = { stdenv, ninja, pkg-config, patchelf, premake5, SDL2, glew, libx11 }:
                let
                  archConfig = if stdenv.isAarch64 then "release_arm64" else "release_x86_64";
                  archLibDir = if stdenv.isAarch64 then "libs/linux/aarch64" else "libs/linux/x86_64";
                in
                stdenv.mkDerivation {
                  pname = "eepp";
                  version = "unstable";
                  src = ./.;

                  nativeBuildInputs = [ premake5 pkg-config patchelf ];
                  buildInputs = [ SDL2 glew libx11 ];

                  configurePhase = ''
                    rm -rf src/thirdparty/efsw
                    rm -rf src/thirdparty/SOIL2
                    cp -rp ${efswPath} src/thirdparty/efsw
                    cp -rp ${soil2Path} src/thirdparty/SOIL2

                    rm -rf premake/premake-ninja
                    rm -rf premake/premake-cmake
                    cp -rp ${premakeNinjaPath} premake/premake-ninja
                    cp -rp ${premakeCMakePath} premake/premake-cmake

                    premake5 --disable-static-build gmake
                  '';

                  buildPhase = ''
                    make -C make/linux config=${archConfig} -j$(nproc)
                  '';

                  installPhase = ''
                    mkdir -p $out
                    cp -R ${archLibDir}/ $out/lib
                    cp -R bin $out/bin

                    find "$out/bin" -type l -lname '/build/*' -delete

                    find "$out/bin" -type f -executable -exec sh -c '
                      file "$1" | grep -q ELF && patchelf --add-rpath "'"$out/lib"'" "$1"
                    ' _ {} \;
                  '';
                };
            in
            { eepp = pkgs.callPackage eepp_pkgs {}; };
        }) systems
      );

      defaultPackage = builtins.listToAttrs (
        map (system: {
          name = system;
          value = self.packages.${system}.eepp;
        }) systems
      );
    };
}